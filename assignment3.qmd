...
```{r}
Grand_Boulevard <- Grand_Boulevard[,-3]
```
...
```{r}
Grand_Boulevard <- as.data.frame(Grand_Boulevard)
...
```{r}
dfs <- list(Grand_Boulevard, east_tables$Hyde_Park, east_tables$Kenwood, east_tables$Oakland)
max_rows <- max(sapply(dfs, nrow))
dfs <- lapply(dfs, function(x) x[seq_len(max_rows), ])

final_table <- cbind(
  dfs[[1]],
  dfs[[2]][, (ncol(dfs[[2]])-1):ncol(dfs[[2]])],
  dfs[[3]][, (ncol(dfs[[3]])-1):ncol(dfs[[3]])],
  dfs[[4]][, (ncol(dfs[[4]])-1):ncol(dfs[[4]])]
)
...
-   Dissimilarity:
    ...
    "Votes" and "cast" are frequently appeared in the website of Grand Boulevard is that this community is politically active, which supported Democratic Party.
